 ...
 *																																			 *
 * Notes: 																															 *
 * ROM libraries are not implemented in this startup code.    					 *
 * Note: C++ destructors of global objects are NOT yet supported in the  *
 * HIWARE Object File Format.	To use this feature, please build your 		 *
 * application with the ELF object file format.													 *
 *************************************************************************/
...
/*************************************************************************
   ...
   Date: 01-07-2006  Author: BB
   
   In _startup (reset vector) remap memory to provide full acess to RAM
   and EEPROM. The PRM RAM segment allocation must coincide with STACK
   initialization.
   
   Project memory map :
   
   REGISTER BASE ADDRESS = 0x0000  INITRG = 0x00
   RAM BASE ADDRESS = 0x2000       INITRM = 0x20
   EEPROM BAS ADDRESS = 0x0800     INITEE = 0x09
...
#if defined(__PROCESSOR_X4__)    /* In some early version DBNE/... did not work. */
#define DO_NOT_USE_TEST_BRANCHES 1 /* Do not use DBNE/TBEQ,.... */
#else
#define DO_NOT_USE_TEST_BRANCHES 0 /* Do use DBNE/TBEQ,.... */
#endif

#pragma DATA_SEG __NEAR_SEG STARTUP_DATA /* _startupData can be accessed */
                                         /* using 16 bit accesses. This  */
                                         /* is needed because it contains*/
                                         /* the stack top, and without 	 */
                                         /* stack, far data cannot 			 */
                                         /* be accessed                  */
...
/* BB added code to remap memory */

asm {
      LDAA #0x00;		// Load 0 into ACC A
      STAA 0x0011;  // Write once to INITRG for safety. Reg default base 0x0000
      
      LDAA #0x20;   // Load 0x20 into A
      STAA 0x0010;  // Remap RAM to 0x2000 base. 0x2000 to 0x3FFF
      
      LDAA #0x09;   //Load 0x09 into A
      STAA 0x0012;  //Remap EEPROM to 0x0800 base
  
} //End of in line assembly
  ...
  for(;;) { /* forever: initialize the program; call the root-procedure */
   ...
   }  /* originally there was a ; after } BB */ /* end loop forever */
}	  /* end startup */
