...
typedef RT_MODEL_q_qube_swingup_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * q_qube_swingup(void);

/* A global buffer for storing error messages (defined in quanser_common library) */
EXTERN char _rt_error_message[512];

/*
   extern void q_qube_swingup_initialize();
   extern void q_qube_swingup_terminate();
 */
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(q_qube_swingup_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(q_qube_swingup_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://q_qube_swingup:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: q_qube_swingup -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(q_qube_swingup_DW.HILReadEncoderTimebase_Task,
        (t_clock) q_qube_swingup_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
